...
public class InsectSpawner : MonoBehaviour
{
    ...
    public float spawnRadiusAroundFlower = 3f;
    public float maxSpawnSearchDistance = 0.5f;
    public int attemptsPerInsect = 10;
    ...
    IEnumerator SpawnRoutine()
    {
        ...
        {
            TrySpawnInsect(transform.position);
            if (delayBetweenSpawns > 0f) yield return new WaitForSeconds(delayBetweenSpawns);
        }
    }

    void TrySpawnInsect(Vector3 flowerPos)
    {
        if (flowerPos.y < -500f) return;

        GameObject prefab = insectPrefabs[UnityEngine.Random.Range(0, insectPrefabs.Length)];

        for (int attempt = 0; attempt < attemptsPerInsect; attempt++)
        {
            Vector2 r = UnityEngine.Random.insideUnitCircle * spawnRadiusAroundFlower;
            Vector3 candidateXZ = new Vector3(flowerPos.x + r.x, 0f, flowerPos.z + r.y);
